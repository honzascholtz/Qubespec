...
def choose_pixels(storage, plot, rad= 0.6, flg=1):
    ...
    if plot==1:
        ...
        plt.imshow(np.ma.array(data=storage['Median_stack_white'], mask=storage['Signal_mask'][0,:,:]), origin='lower')
        plt.colorbar()
...
def D1_spectra_collapse(storage,z, band, plot, addsave=''):
    ...
    np.savetxt(PATH_store+ID+'_'+band+'_1Dspectrum.txt', Save_spec)
...
def stack_sky(storage, Band,plot, spe_ma=np.array([], dtype=bool), expand=0):
    ...
    if plot==1:
        ...
        plt.figure()
        plt.imshow(np.ma.array(data=storage['Median_stack_white'], mask=mask_collapse[0,:,:]), origin='lower')
        plt.colorbar()
