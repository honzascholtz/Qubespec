...
def plotting_general(res, ax, cont_sub = False, OIII_wv = 5008,):
    from astropy.modeling.powerlaws import PowerLaw1D

    if cont_sub:
        OIII_wv = 5008.*(1+res.props['popt'][0])/1e4
        cont = res.props['cont'][0]
        cont_grad = res.props['cont_grad'][0]
        cont_sub = PowerLaw1D.evaluate(res.wave_fitloc, cont,OIII_wv, alpha=cont_grad)

    
    else:
        cont_sub = np.zeros_like(res.flux_fitloc)

    ax.plot(res.wave_fitloc, res.flux_fitloc - cont_sub, drawstyle='steps-mid', alpha=0.99)
    ax.plot(res.wave_fitloc, res.yeval_fitloc - cont_sub, 'r--')
    ax.plot(res.wave_fitloc, res.error_fitloc, 'k:', alpha=0.3)
