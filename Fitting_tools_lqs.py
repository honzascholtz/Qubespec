...
def fitting_Halpha_mul(wave, fluxs, error,z, wvnet=1., decompose=np.array([1]), offset=0,init_sig=300., broad=1, cont=1):
    ...
    if wvnet ==1.:
        wv = float(wave[np.where(wave==wave_peak)[0]] )
    ...
    #
    parameters['Nb_amplitude'].set(expr='Nr_amplitude/3.0')  
    #
    parameters['Nr_sigma'].set(expr='Han_sigma*(6583.0/6562.0)')
    #
    parameters['Nb_sigma'].set(expr='Han_sigma*(6548.0/6562.0)') 
...
def Fitting_OIII_unwrap(lst, wave, z):
    ...
    out, BIC = fitting_OIII_sig(wave,flx_spax_m,error,z)
    
    if i==16 and j==20:
        plt.figure()
        plt.plot(wave, flx_spax_m, color='grey', drawstyle='steps-mid', alpha=0.2)
         
        fluxplt = flx_spax_m.data[np.invert(flx_spax_m.mask)]
        wv_obs = wave[np.invert(flx_spax_m.mask)]
          
          
        plt.plot(wv_obs, fluxplt, drawstyle='steps-mid')
        plt.plot(wave, out.eval(x=wave), 'r--')
    
    prop = {'name': 'OIII'}
    prop['z'] = np.array([  (out.params['o3r_center'].value*1e4/5008)-1])
    prop['cont'] = np.array([out.params['intercept'].value])
    prop['cont_grad'] = np.array([out.params['slope'].value])
    
    prop['OIIIn_peak'] = np.array([out.params['o3r_height'].value])
    prop['OIIIn_fwhm'] = np.array([ out.params['o3r_fwhm'].value/out.params['o3r_center'].value*3e5])
    
    popt = np.array([prop['z'][0],
                     prop['cont'][0],
                     prop['cont_grad'][0],
                     prop['OIIIn_peak'][0],
                     prop['OIIIn_fwhm'][0]])
...
def fitting_CO_mu(wave, fluxs, error, cent, init=[200.,300.],chi2=0):
    ...
    # Narrow line FWHM > min resolution of grating of KMOS (R >~ 3000) < max of 2500 km/s
    parameters['COn_sigma'].set(min=(100.0/2.36/2.9979e5)*cent[0],max=(1000.0/2.36/2.9979e5)*cent[0]) 
    # Velocity offsets between -500 and 500 km/s for narrow
    parameters['COn_center'].set(min=cent[0] + cent[0]*(-400.0/2.9979e5),max=cent[0] + cent[0] *(400.0/2.9979e5))
    
    # Parameters constraints Broad line flux > 0.0
    parameters['COb_amplitude'].set(min=0.0) 
    # Narrow line FWHM > min resolution of grating of KMOS (R >~ 3000) < max of 2500 km/s
    parameters['COb_sigma'].set(min=(100.0/2.36/2.9979e5)*cent[0],max=(1000.0/2.36/2.9979e5)*cent[0]) 
    # Velocity offsets between -500 and 500 km/s for narrow
    parameters['COb_center'].set(min=cent[1] + cent[1]*(-400.0/2.9979e5),max=cent[1] + cent[1] *(400.0/2.9979e5))
...
def fitting_Halpha_mul_testing(wave, fluxs, error,z, wvnet=1., decompose=1, offset=0,init_sig=300., broad=1, cont=1):
    ...
    print(flux.dtype, error.dtype, error.dtype)
    out = model.fit(flux,params=parameters, errors=error, x=wave)
